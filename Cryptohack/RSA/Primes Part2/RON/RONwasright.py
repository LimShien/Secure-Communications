"""
Here's a bunch of RSA public keys I gathered from people on the net together with messages that they sent.

As excerpt.py shows, everyone was using PKCS#1 OAEP to encrypt their own messages. It shouldn't be possible to decrypt them, but perhaps there are issues with some of the keys?
"""
from math import gcd
from Crypto.Cipher import PKCS1_OAEP
from Crypto.PublicKey import RSA
from Crypto.Util.number import GCD,inverse
##an array to store all the keys
allkey =[]
for i in range(1, 51):
    with open("Primes Part2/RON/keynmessage/" + str(i) + ".pem") as f:
        allkey.append(RSA.importKey(f.read()))

#using the fact that  both n use the same factor, so they will have the same factor -> article RON was right
for i in range (0,50):
    for j in range (2,50):
        if(i!=j):
            if(gcd(allkey[i].n,allkey[j].n)!= 1):
                #print("HIT at i = ", i, " j = ", j)
                #print(gcd(allkey[i].n,allkey[j].n))
                pass

##both the 21.pem and 24.pem use the same factor
n = allkey[20].n
p = int(gcd(allkey[20].n,allkey[33].n))
q = int(n//p)
e = allkey[20].e
phi = (p-1)*(q-1)
d = inverse(e, phi)
ct = ""


cipher = PKCS1_OAEP.new(RSA.construct((n,e,d,p,q)))
print(cipher.decrypt(bytes.fromhex(ct)))
##RON is wrong and Whit is right